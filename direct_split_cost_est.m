...
% minimum reflux for an A/BCD split
 r_min = ((RelVol(2)*RelVol(3)*(zF(HK_LK(1))+zF(HK_LK(2))))/(zF(HK_LK(1))*(RelVol(1)*RelVol(2)*RelVol(3)-RelVol(3)*RelVol(2)))) + (RelVol(3)*zF(HK_LK(3))/(zF(HK_LK(1))*(RelVol(1)*RelVol(2)*RelVol(3)-RelVol(3)))) + (zF(HK_LK(4))/(RelVol(1)*RelVol(2)*RelVol(3)-1));
...
% --------------------
[ cost_B_TES_split, cost_check ] = CostModel_func(N_real, V)
% --------------------
%% Begin second column T/ES
disp('Column 2, T/ES:')
...
% minimum reflux for an A/BC split
 r_min = ( (RelVol(3)*(x_F(HK_LK(2))+x_F(HK_LK(3)))) / (x_F(HK_LK(2))*(RelVol(2)*RelVol(3)-RelVol(3))) ) + ( x_F(HK_LK(4)) / (x_F(HK_LK(2))*(RelVol(2)*RelVol(3)-1)) );
...
% shortcut for comparison to different split methods 
% --------------------
[ cost_T_ES_split, cost_check2 ] = CostModel_func(N_real, V)
% --------------------
%% Begin column 3 E/S
disp('Column 3 E/S:')
...
%FOR BINARY -
    ...
    % component
    r_min = (RelVol(3)-1)^-1 * ((x_D(HK_LK(3))/x_F(HK_LK(3))- RelVol(3)*(1-x_D(HK_LK(3))/(1-x_F(HK_LK(3)))))); % (3.57) eqn Doherty
...
% shortcut for comparison to different split methods 
% --------------------
[ cost_E_S_split, cost_check3 ] = CostModel_func(N_real, V)
% --------------------

total_cost = cost_B_TES_split + cost_T_ES_split + cost_E_S_split
