...
% figure
% 	plot(V,EPF,'-','LineWidth',2)
%         axis([0 2 0 2*10^7])
%         ylabel('EP Fuel ($/yr)','FontSize',26)
%         xlabel('Volume (m^3)','FontSize',26)
%         title('PFR; V vs EP Fuel;  T = 793k')
%         legend('MR=8','FontSize',20)
%         set(gca,'FontSize',26)
%    
...
figure
	plot(X,NPV_proj,'-',X,TCI,'-','LineWidth',3);
...
% Plots NPV_percent versus conversion and ROI_BT versus conversion
        ...
        figure
        plot(X,NPV_percent,'-',X,ROI_BT*100,'-','LineWidth',3);
     ylabel('Percent per Year')
       legend('Net Present Value [%/year]' ,'Return on Investment [%/year]') % label left y-axis
